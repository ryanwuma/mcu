...
### ✅ 第五阶段（建议立即执行）：数据外置 & 低门槛更新（无需改代码）

目标：未来新增/修改卡片内容时**不需要改 `index.html`**，只需要填写/更新单独数据文件即可完成全站更新。

**方案 A（先落地，最快见效）— 外部 JSON 驱动**
- **核心思路**：把目前写死在 `index.html` 的 `mcuData` 与 `localVideoFiles` 拆成独立 JSON 文件，前端启动时自动加载。
- **新增文件**：
...
**“MCU 最新动态”一键问题（下一阶段实现）**
- 聊天窗口增加默认按钮：`MCU Latest Updates`
- 优先读取 `news.json`（静态文件，秒回、稳定、省钱），必要时再由模型进行口吻化总结
- 可与 “T.V.A. 裁决协议”联动：自动抓取 → 生成候选 → 人工确认发布 → 自动上线
